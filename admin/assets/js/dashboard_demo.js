...
function setupLeaveFilters() {
  ...
  filterBtn.addEventListener("click", applyLeaveFilters);

  // Load default history
  displayLeaveHistory(getLeaves());
}

function getLeaves() {
  // Example leave data (replace with API or localStorage later)
  return [
    { name: "John Doe", department: "it", type: "Annual", from: "2024-03-15", to: "2024-03-20", status: "Approved" },
    { name: "Sarah Johnson", department: "hr", type: "Sick", from: "2024-04-10", to: "2024-04-12", status: "Pending" },
    { name: "Michael Chen", department: "finance", type: "Casual", from: "2024-05-05", to: "2024-05-06", status: "Rejected" },
  ];
}

function displayLeaveHistory(leaves) {
  const container = document.getElementById("leaveHistoryBody");
  container.innerHTML = "";

  if (!leaves.length) {
    container.innerHTML = `<div class="table-row"><span style="grid-column: 1 / -1; text-align:center; color:#9ca3af;">No leave records found.</span></div>`;
    return;
  }
  ...
  leaves.forEach((leave) => {
    ...
    row.innerHTML = `
      ...
      <span>${formatDepartment(leave.department)}</span>
...
function applyLeaveFilters() {
  const month = document.getElementById("leaveMonthFilter").value;
  const day = document.getElementById("leaveDayFilter").value;
  const year = document.getElementById("leaveYearFilter").value;
  const department = document.getElementById("leaveDepartmentFilter").value;

  let filtered = getLeaves();
